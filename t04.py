...
def main(args):
	...
	nChunks = 1000 #10000 #the size of a file.
...
#	nChunks = int(round(config['duration'] / config['average']))
	...
	# Estimate clip level (if needed)
		...
		jamie         = numpy.zeros((nChunks, 4, LFFT-1))
		...
		for i in xrange(nChunks):
			...
			#jamie[i,:2,:] = (numpy.abs(np.fft.fft2(data[:2,:]))**2/LFFT/2.)
			#jamie[i,0,:] = (numpy.abs(np.fft.fft(data[0,:]))**2/LFFT/2.)
			jamie[i,0,:] = np.abs(np.fft.fftshift(np.fft.fft(data[0,:]))[1:])**2/LFFT
			jamie[i,1,:] = np.abs(np.fft.fftshift(np.fft.fft(data[1,:]))[1:])**2/LFFT
			jamie[i,2,:] = np.abs(np.fft.fftshift(np.fft.fft(data[2,:]))[1:])**2/LFFT
			jamie[i,3,:] = np.abs(np.fft.fftshift(np.fft.fft(data[3,:]))[1:])**2/LFFT
			#jamie[i,1,:] = (numpy.abs(np.fft.fft(data[1,:]))**2/LFFT/2.)
			#jamie[i,2:,:] = (numpy.abs(np.fft.fft2(data[2:,:]))**2/LFFT/2.)
			#jamie[i,2,:] = (numpy.abs(np.fft.fft(data[2,:]))**2/LFFT/2.)
			#jamie[i,3,:] = (numpy.abs(np.fft.fft(data[3,:]))**2/LFFT/2.)
...
#		numpy.savez(outname, freq=freq, freq1=freq+config['freq1'], freq2=freq+config['freq2'], times=masterTimes, spec=masterSpectra, tInt=(maxFrames*4096/beampols/srate), srate=srate,  standMapper=[4*(beam-1) + i for i in xrange(masterSpectra.shape[1])])
		...
		#numpy.save('jamie', jamie)

		for i in range(0,4):
			plt.subplot(2,2,i+1)
			plt.plot((np.log10(jamie[:,i,:].mean(0)))[fcl:fch])
		plt.savefig('jamie1')

		for i in range(0,4):
			plt.subplot(2,2,i+1)
			plt.plot(np.log10(masterSpectra[:,i,:].mean(0)))
		plt.savefig('jamie2')
		...
		for i in range(0,4):
			plt.subplot(2,2,i+1)
			plt.plot( np.log10(masterSpectra[:,i,:].mean(0))/((np.log10(jamie[:,i,fcl:fch].mean(0)))) )
		plt.savefig('jamie3')

		spectarray = masterSpectra[:,:2,:].mean(1)
		spectarray /= np.median(spectarray,0)
