...
if __name__ == '__main__':
    ...
    fcl=360
    fch=3700
    spect=(bandpass(np.asarray((np.load(fn[rank*2+1],mmap_mode='r')[:,:,fcl:fch]),dtype=np.float64))+bandpass(np.asarray((np.load(fn[rank*2+1],mmap_mode='r')[:,:,fcl:fch]),dtype=np.float64)))/2.0
    ts=spect.mean(2)
    bp=spect.mean(0)
    tstotal = ts*0
    bptotal = bp*0
    comm.Allreduce(ts,tstotal,op=MPI.SUM) #merge the 4 hour timeseries from all processor
    comm.Allreduce(bp,bptotal,op=MPI.SUM) #merge the 4 hour timeseries from all processor
    if rank == 0 :
            np.save('ts_std' , tstotal/size) #cut the dispersed time lag
            np.save('bp_std' , bptotal/size) #cut the dispersed time lag
