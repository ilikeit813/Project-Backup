...
if __name__ == '__main__':
    ...
    #for pol in (0,1):
        ...
        tbmax0=0 #repeat control, if dedispersion time series are identical, skip dedispersion calculation
        for ifreqbin in range(4):
            freqi=freq[360+3340/4*ifreqbin:360+3340/4*(ifreqbin+1)]
            tb=np.round((delay2(freqi,DM)/tInt)).astype(np.int32)
            tbmax=tb.max()

            ts=np.zeros((tb.max()+numberofFiles*np.load(fn[0],mmap_mode='r').shape[0]))
            for freqbin in range(3340/4):
                for i in range(fpp):
            ...
            if ifreqbin == 0:
                if rank==0:
                    np.save('tsfreq0',tstotal)
            if ifreqbin == 1:
                if rank==0:
                    np.save('tsfreq1',tstotal)
            if ifreqbin == 2:
                if rank==0:
                    np.save('tsfreq2',tstotal)
            if ifreqbin == 3:
                if rank==0:
                    np.save('tsfreq3',tstotal)
