...
if __name__ == '__main__':
    ...
    nodes =  14 #the number of node requensted in sh
    ...
    stdall=np.load('stdall.npy') #shape=numberofFiles,2 x pol,(2 x stds,2 x mean)=4
    

    for i in range(fpp):
        #spectarray[i] = baseline( bandpass(np.asarray(np.load(fn[rank*fpp+i],mmap_mode='r')[:,:,fcl:fch],dtype=np.float64)))
        #spectarray[i] = baseline( bandpass(np.load(fn[rank*fpp+i])[:,:,fcl:fch]))
        spectarray[i,:,0,:] = np.load(fn[rank*fpp+i])[:,0,fcl:fch]/background(fn[rank*fpp+i])[0]
        spectarray[i,:,1,:] = np.load(fn[rank*fpp+i])[:,1,fcl:fch]/background(fn[rank*fpp+i])[1]
        #for j in range(2):
        #    for pol in range(2):
    ...
    maxLf    = stdall[:,0,0].min()*3
    maxLt    = stdall[:,0,1].min()*3
    medianLf = np.median(stdall[:,0,2])
    medianLt = np.median(stdall[:,0,3])
    maxHf    = stdall[:,1,0].min()*3
    maxHt    = stdall[:,1,1].min()*3
    medianHf = np.median(stdall[:,1,2])
    medianHt = np.median(stdall[:,1,3])

    #print medianHf,medianHt,medianLf,medianLt
    ...
    #        sys.exit()
        ...
        for iDM in range(0,nDMs):
            ...
            #save dedispersed time series.
               ...
               #print 'DM=',DM,'iDM=',iDM
               #np.save('ts_pol%.1i_DM_%.6i' % (pol,iDM),tstotal)
