...
if __name__ == '__main__':
    ...
    bp = np.zeros((fpp, nodes*pps, 2, fch-fcl))
    ...
    for i in range(fpp):
        spectarray[i,:,:,:] = np.load(fn[rank*fpp+i])[:,:,fcl:fch]
        for pol in (0,1):
                bp[i,rank,pol,:]= np.median(spectarray[i,:,pol,:], 0)
    ...
    bp0 = np.median(np.median(bpall[:,:,0,:],0),0)
    bp1 = np.median(np.median(bpall[:,:,1,:],0),0)

    bp0 = savitzky_golay(bp0,151,2)
    bp1 = savitzky_golay(bp1,151,2)
    ...
    for i in range(fpp):
        for pol in (0,1):
                if pol == 0:
                        ...
                        bp = bp1
                spectarray[i,:,pol,:] /= bp
                spectarray[i,:,pol,:] /= np.median(spectarray[i,:,pol,:])
                std[i,rank,pol]=spectarray[i,:,pol,:].std()
    ...
    for i in range(fpp):
        for pol in (0,1):
           spectarray[i,:,pol,:][spectarray[i,:,pol,:]>np.median(spectarray[i,:,pol,:]) + 3*stdall[:,:,pol].min()] = np.median(spectarray[i,:,pol,:])
    '''
