...
def bandpass(sp):
    ...
    for pol in range(2):
        sp[:,pol,np.where(sp[:,pol,:].mean(0)>maxf)]=np.median(sp[:,pol,:].mean(0))
    return(sp[:,0:2,:])

def baseline(sp):
    for pol in range(2):
        sp[:,pol,:] /= np.median(sp[:,pol,:].mean(1))
        sp[np.where(sp[:,pol,:].mean(1)>maxt),pol,:]  = np.median(sp[:,pol,:].mean(1))
    return(sp)
...
if __name__ == '__main__':
    ...
    maxf = 1.06 #RFI cut off in frequency
    maxt = 1.012 #RFI cut off in time domains
    fn=sorted(glob.glob('056*.npy')) 
    tInt=np.load('tInt.npy')

    DMstart=100.0#1.0 #initial DM trial
    DMend  =6000.0#90.0 #finial  DM trial
    ...
    spect=np.append( baseline(bandpass(np.asarray((np.load(fn[rank*fpp],mmap_mode='r')[:,:,fcl:fch]),dtype=np.float64))) , baseline(bandpass(np.asarray((np.load(fn[rank*fpp+1],mmap_mode='r')[:,:,fcl:fch]),dtype=np.float64))),0)
    ...
    for pol in (0,1):
        ...
        for iDM in range(0,nDMs):
            ...
            for freqbin in range(len(freq)): 
                ts[tb.max()-tb[freqbin]+rank*spect.shape[0]:tb.max()-tb[freqbin]+rank*spect.shape[0]+spect.shape[0]]+=spect[:,pol,freqbin] 
            tstotal=ts*0#initiate a 4 hour blank time series
            comm.Allreduce(ts,tstotal,op=MPI.SUM)#merge the 4 hour timeseries from all processor
