...
def RFI(sp):
    ...
    sp[:,np.where( np.abs(sp[:,:].mean(0)-median) > max)] = median
    sp[np.where( np.abs(sp[:,:].mean(1)-median) > max),:] = median
    return sp
...
if __name__ == '__main__':
    ...
    DMstart=10.895#1.0 #initial DM trial
    ...
    max    = stdalltmp[:,0,:].min()*3
    median = np.median(stdalltmp[0,1,:])
    ...
    #for pol in (0,1):
        ...
        np.save('dm10895p%.1i' % pol,tstotal)
