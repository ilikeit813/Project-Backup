...
if __name__ == '__main__':
    comm  = MPI.COMM_WORLD
    rank  = comm.Get_rank()
    fpp   =  30 #spectrogram per processer you want, limited mainly by 64GB memory per node
    nodes = 128 #the number of node requensted in sh
    ...
    DMstart= 2.92#1.0 #initial DM trial
    DMend  = 3.02#90.0 #finial  DM trial
    ...
    for pol in (0,1):
        ...
        for iDM in range(0,nDMs):
            ...
            if rank==0:
               ...
               print 'DM=',DM,'iDM=',iDM
               np.save('ts_pol%.1i_DM_%.10000i' % (pol,iDM),tstotal)
