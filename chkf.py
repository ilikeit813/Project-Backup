...
def main(args):
    ...
    for m in range(l):
      for j in range(len(k)):
       if rank == j + m*nodes*pps:
        offset = k[j]
        print offset
        offsetf = offset - nChunks*nFramesAvg
        outname = glob.glob("%s_*_offset_%.9i_frames.npy" % (getopt.getopt(args,':')[1][0],offsetf))
        spf = numpy.load(outname)
        offsetb = offset + nChunks*nFramesAvg
        outname = glob.glob("%s_*_offset_%.9i_frames.npy" % (getopt.getopt(args,':')[1][0],offsetb))
        spb = numpy.load(outname)
        beam = 5 #combined from others
        outname = "%s_%i_fft_offset_%.9i_frames" % (getopt.getopt(args,':')[1][0], beam,offset)
        numpy.save(outname[-46:],spf/2.+spb/2.) # (x+y) /2
