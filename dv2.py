...
if __name__ == '__main__':
    ...
    DMstart=5.0#1.0 #initial DM trial
    DMend  =15.0#90.0 #finial  DM trial
    ...
    spect=np.asarray((np.load(fn[rank*3],mmap_mode='r')),dtype=np.float64)

    for pol in range(4): 
		RFI=np.load('./rfiflag_%.46s_pol_%.1i_fbins.npy' % (fn[rank*3],pol))-fcl            
		bandpas=(np.load('./bandpass_%.46s_pol_%.1i.npy' % (fn[rank*3/unitbond*unitbond],pol))[fcl:fch])
		spect[:,pol,fcl:fch]=spect[:,pol,fcl:fch]/bandpas
		spect[:,pol,RFI]=1.0

    spect2=np.asarray((np.load(fn[rank*3+1],mmap_mode='r')),dtype=np.float64)

    for pol in range(4): 
		RFI=np.load('./rfiflag_%.46s_pol_%.1i_fbins.npy' % (fn[rank*3+1],pol))-fcl            
		bandpas=(np.load('./bandpass_%.46s_pol_%.1i.npy' % (fn[(rank*3+1)/unitbond*unitbond],pol))[fcl:fch])
		spect2[:,pol,fcl:fch]=spect2[:,pol,fcl:fch]/bandpas
		spect2[:,pol,RFI]=1.0

    spect3=np.asarray((np.load(fn[rank*3+2],mmap_mode='r')),dtype=np.float64)

    for pol in range(4): 
		RFI=np.load('./rfiflag_%.46s_pol_%.1i_fbins.npy' % (fn[rank*3+2],pol))-fcl            
		bandpas=(np.load('./bandpass_%.46s_pol_%.1i.npy' % (fn[(rank*3+2)/unitbond*unitbond],pol))[fcl:fch])
		spect3[:,pol,fcl:fch]=spect3[:,pol,fcl:fch]/bandpas
		spect3[:,pol,RFI]=1.0

    #spect=np.append(np.append(spect,spect2,0),spect3,0)
    ...
    if pol==2:
        ...
        for iDM in range(0,nDMs):
            ...
            for freqbin in range(len(freq)):
                ...
                ts[tb.max()-tb[freqbin]+(rank*3  )*spect.shape[0]:tb.max()-tb[freqbin]+(rank*3  )*spect.shape[0]+spect.shape[0]]+=( spect[:,pol,fcl+freqbin] +spect[:,pol+1,fcl+freqbin])/2.0
                ts[tb.max()-tb[freqbin]+(rank*3+1)*spect.shape[0]:tb.max()-tb[freqbin]+(rank*3+1)*spect.shape[0]+spect.shape[0]]+=(spect2[:,pol,fcl+freqbin]+spect2[:,pol+1,fcl+freqbin])/2.0
                ts[tb.max()-tb[freqbin]+(rank*3+2)*spect.shape[0]:tb.max()-tb[freqbin]+(rank*3+2)*spect.shape[0]+spect.shape[0]]+=(spect3[:,pol,fcl+freqbin]+spect3[:,pol+1,fcl+freqbin])/2.0
