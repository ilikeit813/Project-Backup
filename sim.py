...
if __name__ == '__main__':
    comm = MPI.COMM_WORLD
    rank = comm.Get_rank()
    fpp=680/5#2#spectrogram per processer
    nodes=1#34
    pps=5#10
    numberofFiles=fpp*nodes*pps

    maxpw=0.4*2*16 #Maximum pulse width to search in seconds. default = 1 s.
    dDM = 1.0/(3700-360) #1.0/len(freq)
    thresh=5.0 #SNR cut off
    fn=sorted(glob.glob('simsp*.npy')) 
    tInt=0.4

    DMstart=2000.0#1.0 #initial DM trial
