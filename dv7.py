...
def bandpass(sp):
    ...
    for pol in range(4):
        a=smoth.smooth(sp[:,pol,:].mean(0),ws)
        test=sp[:,pol,:].mean(0)*1.0
        for i in range(1):
            test[np.where(test/a>1)]=a[np.where(test/a>1)]
            a=smoth.smooth(test,ws)
        sp[:,pol,:]/=a
        sp[:,pol,np.where(sp[:,pol,fcl:fch].mean(0)>max)]=np.median(sp[:,pol,fcl:fch].mean(0))
    return(sp)

def baseline(sp):
    for pol in range(4):
        sp[:,pol,fcl:fch] /= np.median(sp[:,pol,fcl:fch].mean(1))
        sp[np.where(sp[:,pol,fcl:fch].mean(1)>max),pol,fcl:fch]  = np.median(sp[:,pol,fcl:fch].mean(1))
    return(sp)
...
if __name__ == '__main__':
    ...
    max = 1.025 #RFI cut off in frequency and time domains

    fn=sorted(glob.glob('056*.npy')) 
    tInt=np.load('tInt.npy')

    DMstart=1.0#1.0 #initial DM trial
    DMend  =3000.0#90.0 #finial  DM trial
    ...
    spect=np.append( baseline(bandpass(np.asarray((np.load(fn[rank*fpp],mmap_mode='r')),dtype=np.float64))) , baseline(bandpass(np.asarray((np.load(fn[rank*fpp+1],mmap_mode='r')),dtype=np.float64))),0)
    ...
    #    if rank == 0 :
    #        np.save('ts_p%.1d' % pol, tstotal) #cut the dispersed time lag
    #sys.exit()
    #======================================================================================================
    ...
    for pol in range(4):
        ts=np.zeros((numberofFiles*np.load(fn[0],mmap_mode='r').shape[0]))
        ts[ra==nk*spect.shape[0]:rank*spect.shape[0]+spect.shape[0]] += spect[:,pol,fcl:fch].mean(1)
        tstotal=ts*0 #initiate a 4 hour blank time series
        comm.Allreduce(ts,tstotal,op=MPI.SUM) #merge the 4 hour timeseries from all processor
        if rank == 0 :
            np.save('ts_p%.1d' % pol, tstotal) #cut the dispersed time lag
