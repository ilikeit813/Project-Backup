...
if __name__ == '__main__':
    comm = MPI.COMM_WORLD
    rank = comm.Get_rank()
    fpp  =3#spectrogram per processer
    nodes=20
    pps  =16#processer per node
    ...
    DMstart=8.18#1.0 #initial DM trial
    DMend  =10.18#90.0 #finial  DM trial
    ...
    for pol in (0,1):
        ...
        for iDM in range(0,nDMs):
            ...
            if rank<npws:#timeseries is ready for signal search
                ranki=rank
                filename = "v0_pp_SNR_pol_%.1i_td_%.2i_no_%.05i.txt" % (pol,ranki,txtsize[ranki,0])
