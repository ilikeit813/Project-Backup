...
def main(args):
    ...
    pol = tunes
    nodes =  4 #89 #the number of node requensted in sh
    ...
    freq1 = np.load('freq1.npy')
    freq2 = np.load('freq2.npy')
    ...
    temp = np.load(fn[0],mmap_mode='r')
    spectarray = np.zeros((temp.shape[0]*len(fn)/nodes/pps, temp.shape[1])) # X and Y are merged already after bandpass
    #cobimed spectrogram and remove background
    for fpp in range(len(fn)/nodes/pps):
        temp = np.load(fn[rank*len(fn)/nodes/pps + fpp])
        spectarray[fpp*temp.shape[0] : (fpp+1)*temp.shape[0],:] = temp/temp.mean()
