...
def bandpass(sp):
    ...
    for pol in range(2):
      if pol == 0:
        sp[:,pol,np.where(sp[:,pol,:].mean(0)>maxLf)]=np.median(sp[:,pol,:].mean(0))
        sp[np.where(sp[:,pol,:].mean(1)>maxLt),pol,:]=np.median(sp[:,pol,:].mean(1))
      else:
        sp[:,pol,np.where(sp[:,pol,:].mean(0)>maxHf)]=np.median(sp[:,pol,:].mean(0))
        sp[np.where(sp[:,pol,:].mean(1)>maxHt),pol,:]=np.median(sp[:,pol,:].mean(1))
    return(sp[:,0:2,:])
...
if __name__ == '__main__':
    ...
    dDM = 1.0/500 #1/(3700-360) #1.0/len(freq)
    ...
    #    if rank == 0 :
    #        np.save('ts_p%.1d_v1' % pol, tstotal) #cut the dispersed time lag
    #sys.exit()
    #======================================================================================================
