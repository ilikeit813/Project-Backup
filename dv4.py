...
if __name__ == '__main__':
    ...
    DMstart=10.90#1.0 #initial DM trial
    DMend  =10.904#90.0 #finial  DM trial
    ...
    spect=np.asarray((np.load(fn[rank*fpp],mmap_mode='r')),dtype=np.float64)[:,:,fcl:fch]

    for pol in range(4): 
		RFI=np.load('./rfiflag_%.46s_pol_%.1i_fbins.npy' % (fn[rank*fpp],pol))-fcl            
		bandpas=(np.load('./bandpass_%.46s_pol_%.1i.npy' % (fn[rank*fpp/unitbond*unitbond],pol))[fcl:fch])
		spect[:,pol,:]=spect[:,pol,:]/bandpas
		spect[:,pol,RFI]=1.0

    #"""
    spect2=np.asarray((np.load(fn[rank*fpp+1],mmap_mode='r')),dtype=np.float64)[:,:,fcl:fch]

    for pol in range(4): 
		RFI=np.load('./rfiflag_%.46s_pol_%.1i_fbins.npy' % (fn[rank*fpp+1],pol))-fcl
		bandpas=(np.load('./bandpass_%.46s_pol_%.1i.npy' % (fn[(rank*fpp+1)/unitbond*unitbond],pol))[fcl:fch])
		spect2[:,pol,:]=spect2[:,pol,:]/bandpas
		spect2[:,pol,RFI]=1.0
    ...
    """
    spect2=np.asarray((np.load(fn[rank*fpp+2],mmap_mode='r')),dtype=np.float64)[:,:,fcl:fch]

    for pol in range(4): 
		RFI=np.load('./rfiflag_%.46s_pol_%.1i_fbins.npy' % (fn[rank*fpp+2],pol))-fcl
		bandpas=(np.load('./bandpass_%.46s_pol_%.1i.npy' % (fn[(rank*fpp+2)/unitbond*unitbond],pol))[fcl:fch])
		spect2[:,pol,:]=spect2[:,pol,:]/bandpas
		spect2[:,pol,RFI]=1.0
